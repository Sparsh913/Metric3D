...
class MatterPort3DDataset(BaseDataset):
            ...
            for line in f:
                ...
                if not self.benchmark:  # benchmark test
                    ...
                    img_info["annotation_filename_depth"] = os.path.join(
                        self.base_path, 'M3D_high', depth_map
                    )
                img_name = line.strip().split(" ")[0]
                img_info["image_filename"] = os.path.join(self.base_path, 'M3D_high' ,img_name)
    ...
    def __getitem__(self, idx):
        ...
        if self.test_mode:
            return {"image": image, "gt": gts["gt"], "mask": gts["mask"], "lat_range": lat_range, "long_range":long_range, "info": info, "intrinsic": [1/np.tan(np.pi/512), 1/np.tan(np.pi/512), 256., 512.]}
        else:
