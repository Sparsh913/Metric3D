...
from mono.utils.erp_geometry import fisheye_kb_to_erp, cam_to_erp_patch_fast
...
class ScanNetPPERPDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.test_mode:
            return {"image": image, "gt": gts["gt"], "mask": gts["mask"], "attn_mask": gts["attn_mask"], "lat_range": lat_range, "long_range":long_range, "info": info, "intrinsic": [1 / np.tan(np.pi/self.cano_sz[1]), 1 / np.tan(np.pi/self.cano_sz[0]), self.cano_sz[1]/2, self.cano_sz[0]/2]}
        else:
